...
MIB-circuit-track is OUR FORK (with the EAP-IG submodule), vendored at `deps/MIB-circuit-track`
(anonymised snapshot of the commit pinned below; the fork URL is withheld for review). Upstream
...
Resolution order, first hit wins:
  ...
  2. `$MATTR_MIB_PATH`
  3. `<repo>/deps/MIB-circuit-track` (vendored; cloned from $MATTR_MIB_URL at the pin if absent)
"""
...
MIB_URL = ""  # withheld for anonymous review; the fork is vendored in deps/MIB-circuit-track
# fork main, 2026-09-18: EAP-IG @ 3fb341e (AtP / AtP-star node methods,
...
def _clone(dest: Path) -> None:
    ...
    if not url:
        raise FileNotFoundError(f"{dest} is missing and no fork URL is set; restore the vendored "
                                "deps/MIB-circuit-track or set $MATTR_MIB_URL")
