...
> **Anonymous submission.** This is an anonymised snapshot of the code for review. Author names, repository/HuggingFace links and cluster-specific details have been removed or replaced by placeholders; everything else is as run.
...
| `deps/MIB-circuit-track/` | Vendored snapshot of **our fork** of MIB-circuit-track (with the EAP-IG submodule) at the pinned commit; the fork's URL is withheld for review. Found by `deps.find_mib_path`. |
| `results/` | One directory per run, `<variant>_<optim>_lr_<lr>/<task>_<model>_<split>.pkl`; gitignored. |


## Experiments

Every group has python entry points at its top level, cluster launchers (`submit_*.sh`, `*.sbatch`) under `launch/`, and is run from the repo root. The launchers are kept as run, with site details anonymised: `gpujob` stands for a site-specific SLURM wrapper (`-g` GPUs, `-q` queue, `-d` GPU type, `-r` memory, `-c` CPUs, `-n` job name), and "cluster A" / "cluster B" name the two clusters used.
...
We use `uv` for environments. `scripts/setup.sh` checks the vendored MIB fork in `deps/`, syncs the default environment and runs a no-model smoke check.

```bash
# from the root of the anonymised checkout
