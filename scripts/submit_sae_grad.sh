...
#
#   SMOKE=1 bash scripts/submit_sae_grad.sh   # 2 jobs: ig + ixg, nounpp, logit_diff
#   bash scripts/submit_sae_grad.sh           # the full 48 (8 tasks x 2 methods x 3 losses)
#   DRY=1 ...                                 # print only
...
sub_one() {  # $1=jobname $2=predicted-filename ; rest = eval_sva args
  ...
  if [[ "${DRY:-0}" == 1 ]]; then echo "  $name -> $f"; else
    sbatch -J "$name" sva_sweep.sbatch "$@" --output "$OUT" >/dev/null; fi
  n=$((n+1))
}
