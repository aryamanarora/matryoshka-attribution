...
# Resolved through deps.find_mib_path (deps/MIB-circuit-track/results on the canonical checkout; the cluster-A path
# above as the last resort). Hardcoded to cluster A until 2026-09-16, which on cluster B dropped every row
...
def main():
    ...
    # === Node-level section ===
    # Every node-level baseline in this section (NAP-IG and its step variants, the cluster A
    ...
    SUBSET_LLAMA3_DAGGER = {(t, m) for t, m, _ in COLUMNS if m == "llama3"}
    ...
    DAGGER["NAP-IG"] = SUBSET_LLAMA3_DAGGER
    ...
    for disp, dirn, _cost in NAPIG_STEP_ROWS:
        ...
        DAGGER[disp] = SUBSET_LLAMA3_DAGGER

    # Additional baselines fetched from cluster A (node-level); each dir has one method subfolder
    ...
    if eg:
        ...
        DAGGER["Expected Gradients"] = SUBSET_LLAMA3_DAGGER
    ...
    # cluster A baselines used a reduced subset for the llama3 cells only -> dagger those
    # (SUBSET_LLAMA3_DAGGER is defined at the top of this section).
    ...
    for name, data, dirn in eprun_rows("node"):
        ...
        DAGGER[name] = SUBSET_LLAMA3_DAGGER
    ...
    # SUBSET_LLAMA3_DAGGER assignment comes from. Overriding here rather than editing the static
    # dict keeps the two fixes side by side with their sections.
    DAGGER["EAP-IG-inp (CF, repro)"] = SUBSET_LLAMA3_DAGGER
    ...
    # MAttr edge llama3 cells use a reduced eval subset (large-memory rerun) -> dagger.
