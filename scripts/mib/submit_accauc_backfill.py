...
cells that have a saved circuit but no acc_auc, and submits scripts/mib/reeval_mib_accauc.py.
...
deliberately cannot paper over (see the printed TRAIN lines; scripts/mib/launch/submit_edge_arc_llama3.sh
...
so re-running early duplicates work rather than extending it.
  ...
  uv run python scripts/mib/submit_accauc_backfill.py            # submit
  DRYRUN=1 uv run python scripts/mib/submit_accauc_backfill.py   # print the plan only
"""
import os
import pickle
import subprocess
