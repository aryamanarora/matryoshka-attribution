#!/bin/bash
# Rerun edge-level L2A ablations train->val (honest). Run from repo root on cluster B in tmux.
...
for a in "${ABL[@]}"; do
  ...
  for p in "${PAIRS[@]}"; do
    ...
    gpujob -g 1 -q gpu -d a6000 $res -n "$name" \
      "PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True uv run python scripts/mib/eval_mib_edge.py --model $model --task $task --steps 5000 --k-schedule log --masking $mask --mode iso --split validation --train-split train --batch-size $bs --eval-examples 0 --output results/$out"
    sleep 1
  done
done
