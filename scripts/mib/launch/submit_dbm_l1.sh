...
#
#   bash scripts/mib/launch/submit_dbm_l1.sh              # submit all 55
#   bash scripts/mib/launch/submit_dbm_l1.sh 2.0          # just one coefficient
#   DRYRUN=1 bash scripts/mib/launch/submit_dbm_l1.sh     # preview
#   L1_TARGET=logit bash scripts/mib/launch/submit_dbm_l1.sh   # the mask tutorial's term instead
#
  ...
  for p in "${PAIRS[@]}"; do
    ...
    if [ "$DRYRUN" = "1" ]; then
      ...
      GATE=sigmoid LOSS=logit_diff LR="$LR" L1="$c" L1_TARGET="$L1_TARGET" \
        ...
        scripts/mib/launch/run_edge_pruning.sbatch "$model" "$task" node 3000 validation >/dev/null \
        && echo "submitted $name"
    fi
    n=$((n+1))
  done
