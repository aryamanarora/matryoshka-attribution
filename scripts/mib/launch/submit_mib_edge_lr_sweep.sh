...
#    Gemma-2 forward, 525673a). scripts/mib/reeval_gemma_mib.py re-EVALUATED those cells on 07-24,
...
# (our account association, GrpTRES gres/gpu=8), so all 108 jobs at once is ~31 h wall-clock, not one
# night. SLURM runs this QOS roughly FIFO, so submission order IS priority order:
#
#   for m in gpt2 llama3 qwen2.5 gemma2; do ONLY=$m bash scripts/mib/launch/submit_mib_edge_lr_sweep.sh; done
#
