...
#
#   bash scripts/mib/launch/submit_node_pruning_sparsity.sh            # 0.8 and 0.5, 22 jobs
#   bash scripts/mib/launch/submit_node_pruning_sparsity.sh 0.8         # just one budget
#   DRYRUN=1 bash scripts/mib/launch/submit_node_pruning_sparsity.sh    # preview
#   LOSS=logit_diff bash scripts/mib/launch/submit_node_pruning_sparsity.sh 0.9   # MAttr's objective, _ld dirs
#
  ...
  for p in "${PAIRS[@]}"; do
    ...
    if [ "$DRYRUN" = "1" ]; then
      ...
      LOSS="$LOSS" sbatch --job-name="$name" \
        scripts/mib/launch/run_edge_pruning.sbatch "$model" "$task" node 3000 validation "$s" >/dev/null \
        && echo "submitted $name"
    fi
    n=$((n+1))
  done
