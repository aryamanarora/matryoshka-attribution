...
DRYRUN=${DRYRUN:-0}; LRS=${LRS:-"0.005 0.01 0.02 0.1 0.2"}; CELLS=${CELLS:-"gpt2:ioi qwen2.5:ioi"}
n=0
for lr in $LRS; do for c in $CELLS; do
  IFS=: read -r model task <<< "$c"
  out=results/test_node_topk_uniform_lr${lr}_zero
  [ -f "$out/${task}_${model}_scores.pt" ] && continue
  name="zlr$lr-$task-$model"
...
--include-input --ablation zero --output $out"
  n=$((n+1))
