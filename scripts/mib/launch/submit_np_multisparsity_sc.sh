...
# The analogue of submit_dbm_multisparsity.sh (cluster A/sbatch) for cluster B/gpujob; run INSIDE tmux.
...
# llama3 on large-memory h100 (the ARC test splits at batch 2 are the long ones -- ~7 h on cluster A);
...
for cell in "${CELLS[@]}"; do
  ...
  case "$model" in
    ...
    llama3)       res="-q gpu-big -d h100 -r 128G" ;;
  esac
