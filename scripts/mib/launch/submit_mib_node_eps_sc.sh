...
#   test_node_topk_uniform_lr05_eps1e-2  (test)
# gemma2 cells run under TL 2.15.4 (the `tl2` group, own .venv-tl2) and are opt-in via MODELS.
# sc / nlprun, run INSIDE tmux.
#   bash scripts/mib/launch/submit_mib_node_eps_sc.sh
#   MODELS=gemma2 bash scripts/mib/launch/submit_mib_node_eps_sc.sh
set -u
ABS=${ABS:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}; cd "$ABS"; mkdir -p logs
DRYRUN=${DRYRUN:-0}; SPLITS=${SPLITS:-"validation test"}; EPS=${EPS:-1e-2}
...
for split in $SPLITS; do
  [ "$split" = validation ] && out="mib_node_topk_uniform_lr05_eps$EPS" || out="test_node_topk_uniform_lr05_eps$EPS"
  for p in "${PAIRS[@]}"; do
    read -r model task <<< "$p"
    ...
    name="eps$EPS-${split:0:3}-${task}-${model}"
    cmd="$EXP $py scripts/mib/eval_mib.py --model $model --task $task --steps 500 --k-schedule uniform \
--masking topk --optimizer adam --adam-eps $EPS --mode sufficient --lr 0.05 --split $split --train-split train \
...
echo "== $([ "$DRYRUN" = 1 ] && echo 'DRY ')total $n eps=$EPS jobs, $skip skipped =="
