...
for lr in 0.005 0.01 0.05 0.1 0.3; do
  for p in "${ALL[@]}"; do
    ...
    # scripts/mib/launch/mib_node_seed.sbatch: <model> <task> <masking> <ks> <outdir> <opt> <seed> <bs> <lr>
    sbatch $RES -J iadamlr_${lr}_${model}_${task} scripts/mib/launch/mib_node_seed.sbatch \
