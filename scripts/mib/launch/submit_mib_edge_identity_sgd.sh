...
# k-schedules. Reproduces jobs from 2026-07-01. Run from repo root on cluster A (SLURM).
#
...
# submit_mib_edge_ablations.sh vs submit_bigmem_llama_edge.sh):
...
for SCHED in uniform log; do
  ...
  for p in "${PAIRS[@]}"; do
    ...
    # scripts/mib/launch/mib_edge.sbatch args: <model> <task> <masking> <kschedule> <outdir> <opt> <seed> <bs> <eval-examples>
    sbatch $RES -J eidsgd_${tag}_${model}_${task} scripts/mib/launch/mib_edge.sbatch \
