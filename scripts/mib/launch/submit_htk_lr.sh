...
for lr in 0.005 0.05 0.1 0.3; do
  nlprun -g 1 -q jag -d a6000 -c 2 -r 32G -n "htk-lr$lr" \
    "$EXP uv run python scripts/mib/eval_mib.py --model gpt2 --task ioi --steps 500 --k-schedule uniform --masking hard_topk --mode sufficient --lr $lr --split validation --train-split train --include-input --output results/htk_lr_$lr"
  sleep 1
done
