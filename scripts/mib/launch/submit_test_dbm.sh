...
#
#   bash scripts/mib/launch/submit_test_dbm.sh 0.3 6.0     # results/eprun_node_ld_sig_lr0.3_l16.0
#   bash scripts/mib/launch/submit_test_dbm.sh 0.3         # unpenalised lr=0.3
#   DRYRUN=1 bash scripts/mib/launch/submit_test_dbm.sh 0.3 6.0
#
...
for p in "${PAIRS[@]}"; do
  ...
  if [ "$DRYRUN" = "1" ]; then
    ...
    env EVAL_ONLY=1 GATE=sigmoid LOSS=logit_diff LR="$LR" ${L1:+L1="$L1"} \
      ...
      scripts/mib/launch/run_edge_pruning.sbatch "$model" "$task" node 3000 test >/dev/null \
      && echo "submitted $name"
  fi
  n=$((n+1))
done
