...
#
#   bash scripts/mib/launch/submit_sigmoid_mask.sh            # 11 cells at pyvene's lr=1e-3
#   LR=0.3 bash scripts/mib/launch/submit_sigmoid_mask.sh     # 11 cells at the swept lr
#   DRYRUN=1 bash scripts/mib/launch/submit_sigmoid_mask.sh   # preview
#
# LR is the tuned knob: pyvene's published 1e-3 averages 0.61 CPR AUC on the three cheap
# cells and 0.3 averages 1.13 (see scripts/mib/launch/submit_sigmoid_mask_lr.sh for why we sweep it and
...
for p in "${PAIRS[@]}"; do
  ...
  if [ "$DRYRUN" = "1" ]; then
    ...
    GATE=sigmoid LOSS=logit_diff LR="$LR" sbatch --job-name="$name" \
      scripts/mib/launch/run_edge_pruning.sbatch "$model" "$task" node "$STEPS" validation >/dev/null \
      && echo "submitted $name"
  fi
  n=$((n+1))
done
