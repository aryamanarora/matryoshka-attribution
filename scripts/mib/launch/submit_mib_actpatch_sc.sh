...
# noise pkl exists is skipped, so the wave is resumable. cluster B / gpujob, run INSIDE tmux.
...
for split in $SPLITS; do
  ...
  for p in "${PAIRS[@]}"; do
    ...
    py="uv run python"; ec=""
    case $model in
      gpt2|qwen2.5) res="-q gpu -d a6000 -c 2 -r 32G"; bs=20; ebs=20 ;;
      gemma2)       res="-q gpu -d a6000 -c 3 -r 64G"; bs=10; ebs=4; py=$PY_GEMMA ;;
      llama3)       res="-q gpu -d a6000 -c 4 -r 96G"; bs=10; ebs=2 ;;
    esac
