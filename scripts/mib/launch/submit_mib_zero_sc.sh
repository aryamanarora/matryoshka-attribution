...
# gemma2 in the tl2 env (opt-in via MODELS); llama3 ARC on h100. cluster B / gpujob, run INSIDE tmux.
...
# BIG="-q gpu-big -d h200 -r 128G" for the 141 GB h200s ; note "-d a100" can land on
# some nodes, which are 40 GB and OOM these cells -- prefer the h200 override (the only h100,
# killed every job step with "Unable to satisfy cpu bind request" on 2026-09-18).
BIG=${BIG:-"-q gpu-big -d h100 -r 128G"}
...
sub() {  # name res cmd
  n=$((n+1))
  if [ "$DRYRUN" = 1 ]; then echo "DRY gpujob -g 1 $2 -n $1"; echo "    $3"; else
    gpujob -g 1 $2 -n "$1" -o "$ABS/logs/$1.out" "$3" 2>&1 | grep -E 'Submitted batch job' | sed "s/^/$1: /"; sleep 1
  fi
}
for cell in "${CELLS[@]}"; do
  ...
  case $model in
    ...
    gemma2)       res="-q gpu -d a6000 -c 3 -r 64G"; py=$PY_G; pye=$PYE_G; tbs="--batch-size 4"; ebs=4 ;;
    llama3)       res="-q gpu -d a6000 -c 4 -r 96G"; tbs="--batch-size 2"; ebs=2 ;;
  esac
