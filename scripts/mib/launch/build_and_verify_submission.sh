...
# the end). Run as a cluster job -- on cluster B the login node's NFS makes torch imports crawl:
#
#   gpujob -q gpu -g 1 -c 4 -r 48G -n mib-build --dependency afterok:<edge job> \
...
NEXT, manual (publishes):
  uv run hf upload <hf-user>/<repo> $OUT . --repo-type model     # repo must be PUBLIC
