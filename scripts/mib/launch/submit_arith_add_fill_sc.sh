...
# arithmetic_addition / llama3, TEST split. sc / nlprun, run INSIDE tmux (nlprun is not on PATH
...
#                   (all: --include-input --batch-size 2, full test split; jag a6000)
...
#                   (all: --batch-size 2 --eval-examples 200 = the $\dagger$ cap; sphinx h100)
...
NODE_RES="-q jag -d a6000 -c 4 -r 96G"          # 8B at batch 2 fits 48 GB (headline node run did)
EDGE_RES="-q sphinx -d h100 -r 128G"            # edge / DBM training need 80 GB (submit_sphinx_retry.sh)
...
submit() {  # name resources command  -> prints job id on stdout (empty on dry run)
  ...
  n=$((n+1))
  if [ "$DRYRUN" = "1" ]; then echo "DRY nlprun -g 1 $res -n $name" >&2; echo "    $cmd" >&2; echo ""; return; fi
  local out; out=$(nlprun -g 1 $res -n "$name" -o "$ABS/logs/${name}.out" "$cmd" 2>&1)
