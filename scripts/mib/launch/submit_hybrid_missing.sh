...
for p in "${PAIRS[@]}"; do
  ...
  nlprun -g 1 -q jag -d a6000 $res -n "hyb2-${task}-${model}" \
    "PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True uv run python scripts/mib/eval_hybrid_scores.py --model $model --task $task --split validation --batch-size $bs --output results/hybrid_eval"
  sleep 1
done
