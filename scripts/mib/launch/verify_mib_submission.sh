...
# need the `tl2` group instead (README.md). On cluster B, run this as a CPU job, not on the login
...
[[ "$SUB" = /* ]] || SUB="$ABS/$SUB"
MIB=$(cd "$ABS" && uv run python -c "from matryoshka_attribution.deps import find_mib_path; print(find_mib_path())")
