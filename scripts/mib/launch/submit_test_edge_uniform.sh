...
for p in "${PAIRS[@]}"; do
  ...
  nlprun -g 1 -q jag -d a6000 $res -n "teu-${task}-${model}" \
    "PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True uv run python scripts/mib/eval_mib_edge.py --model $model --task $task --steps 5000 --k-schedule uniform --masking hard_topk --mode sufficient --split test --train-split train --batch-size 5 --eval-examples 0 --output results/test_edge_hard_topk_uniform"
  sleep 1
done
