...
# wrong, README.md). llama3 ARC on the h100. cluster B / gpujob, run INSIDE tmux.
...
for cell in "${CELLS[@]}"; do
  ...
  tdash=${task//_/-}
  pye=$PYE_L2A
  case $model in   # eval batch sizes as submit_mib_zero_sc.sh (the last cluster B wave through this runner)
    gpt2|qwen2.5) res="-q gpu -d a6000 -c 2 -r 32G"; ebs=20 ;;
    gemma2)       res="-q gpu -d a6000 -c 3 -r 64G"; ebs=4; pye=$PYE_G ;;
    llama3)       res="-q gpu -d a6000 -c 4 -r 96G"; ebs=2 ;;
  esac
  ...
  [ "$model" = llama3 ] && { [[ "$task" == arc_* ]] || [ "$task" = mcqa ]; } && res="${BIG:--q gpu-big -d h100 -r 128G}"
