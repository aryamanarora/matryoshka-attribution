...
#  - small models (gpt2/qwen/gemma): only bernoulli_reinforce row was never run -> gpu queue, full eval.
#  - llama3 cells: a6000 OOMs at full eval -> large-memory h100, 200-example subset (daggered).
set -u
cd "$(dirname "${BASH_SOURCE[0]}")/../../.."
EXP="PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True"

# ---- small-model bernoulli_reinforce cells (a6000, full eval) ----
for p in "gpt2 ioi" "qwen2.5 ioi" "gemma2 ioi" "qwen2.5 mcqa" "gemma2 mcqa" "gemma2 arc_easy"; do
  ...
  gpujob -g 1 -q gpu -d a6000 $res -n "eb-${task}-${model}" \
...
# ---- llama3 cells (large-memory h100, subset 200) ----
# spec: <output-dir> <masking> <tasks...>
run_llama() {
  out=$1; mask=$2; shift 2
  for t in "$@"; do
    gpujob -g 1 -q gpu-big -d h100 -r 128G -n "el-${mask}-${t}" \
