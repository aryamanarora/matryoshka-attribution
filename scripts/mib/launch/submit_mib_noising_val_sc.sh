...
# nlprun job per (objective, cell): scripts/mib/eval_mib_noising.py -> <dir>/<task>_<model>_
# validation_noising.pkl. sc, run INSIDE tmux. Eval only (the rankings exist).
#
...
for obj in $OBJS; do
  ...
  for cell in "${CELLS[@]}"; do
    ...
    case "$model" in
      ...
      llama3)       res="-q jag -d a6000 -c 4 -r 96G"; head="--head 200" ;;
    esac
    ...
    n=$((n+1))
    if [ "$DRYRUN" = 1 ]; then echo "DRY nlprun -g 1 $res -n $name"; else
