#!/bin/bash
# cluster B twin of submit_arith_add_headline.sh: same two arithmetic_addition/llama3
# cells for the headline MAttr config, submitted with gpujob instead of raw sbatch. Same
# protocol and output dirs -- see that script's header. Run INSIDE the tmux session (gpujob is
# not on PATH over plain ssh).
#
# GPU choice follows the cluster B precedent for llama3: node level fits a 48GB a6000 (README.md:
# -d a6000 -c 4 -r 96G --batch-size 2); edge level at 5000 steps needs 80GB, i.e. large-memory h100
# (submit_bigmem_retry.sh -- the 40GB a100s OOM'd). gpujob's default time limit is 10 days.
#
...
set -u
ABS=${ABS:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}; cd "$ABS"; PY="uv run python"   # uv syncs the env inside the job
DRYRUN=${DRYRUN:-0}
SPLITS=${SPLITS:-test}
...
--batch-size 2 --eval-examples 200 --output results/$out" ;;
    ...
    if [ "$DRYRUN" = "1" ]; then echo "DRY gpujob -g 1 $res -n $name -> results/$out"; echo "    $cmd"; else
      gpujob -g 1 $res -n "$name" -o "$ABS/logs/${name}.out" "$cmd" && echo "submitted $name -> results/$out"
      sleep 1
    fi
    n=$((n+1))
  done
...
echo "== ${pfx}total $n arithmetic_addition headline jobs (cluster B), $skip skipped =="
