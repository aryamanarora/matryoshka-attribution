...
# cluster-A-era launcher this copies, paths and env swapped for cluster B). Hard-concrete gates, the original
...
# opt-in via MODELS. cluster B / gpujob, run INSIDE tmux.
...
for s in $SPARSITIES; do
  ...
  for split in $SPLITS; do for p in "${PAIRS[@]}"; do
    ...
    case $model in
      gpt2|qwen2.5) res="-q gpu -d a6000 -c 3 -r 64G"; bs=20 ;;
      gemma2)       res="-q gpu -d a6000 -c 4 -r 96G"; bs=4
      ...
      llama3)       res="-q gpu-big -d h100 -r 128G"; bs=2; head="--head 200" ;;
    esac
