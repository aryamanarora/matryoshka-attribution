...
CDIR=results/eapig_mc                       # relative to $MIB (where the circuits live)
OUT=$ABS/results/eapig_mc_$SPLIT
PAIRS=(
...
for p in "${PAIRS[@]}"; do
  ...
  tdash=${task//_/-}
  [ -d "$MIB/$CDIR/EAP-IG-inputs-mc_patching_edge/${tdash}_${model}" ] || { echo "MISSING circuit ${tdash}_${model}"; continue; }
  if [ -f "$OUT/EAP-IG-inputs-mc_patching_edge/${tdash}_${model}_${SPLIT}_abs-False.pkl" ]; then
...
--method EAP-IG-inputs-mc --level edge --ablation patching --split $SPLIT --batch-size $bs $head \
--circuit-dir $CDIR --output-dir $OUT"
  name="egedge-${SPLIT:0:3}-${task}-${model}"
  n=$((n+1))
...
echo "== $([ "$DRYRUN" = 1 ] && echo 'DRY ')total $n edge Expected-Gradients $SPLIT evals, $skip skipped =="
