...
# `tl2` env and is opt-in via MODELS. cluster B / gpujob, run INSIDE tmux.
...
for p in "${PAIRS[@]}"; do
  ...
  case $model in
    gpt2)    res="-q gpu -d a6000 -c 3 -r 64G"; bs=10 ;;
    qwen2.5) res="-q gpu -d a6000 -c 3 -r 64G"; bs=5 ;;
    gemma2)  res="-q gpu -d a6000 -c 4 -r 96G"; bs=1
             pye="UV_PROJECT_ENVIRONMENT=$ABS/.venv-tl2 uv run --no-default-groups --group tl2 --project $ABS python" ;;
    llama3)  res="-q gpu-big -d h100 -r 128G"; bs=1; head="--head 200" ;;
  esac
