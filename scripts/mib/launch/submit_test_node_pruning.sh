...
#
#   bash scripts/mib/launch/submit_test_node_pruning.sh                       # KL, s=0.9  (results/eprun_node)
#   LOSS=logit_diff bash scripts/mib/launch/submit_test_node_pruning.sh 0.5   # results/eprun_node_s0.5_ld
#
...
for p in "${PAIRS[@]}"; do
  ...
  if [ "$DRYRUN" = "1" ]; then
    ...
    EVAL_ONLY=1 LOSS="$LOSS" sbatch --job-name="$name" \
      scripts/mib/launch/run_edge_pruning.sbatch "$model" "$task" node 3000 test $SPARSITY >/dev/null \
      && echo "submitted $name"
  fi
  n=$((n+1))
done
