...
for p in "${PAIRS[@]}"; do
  ...
  nlprun -g 1 -q jag -d a6000 $res -n "h2k-${task}-${model}" \
    "$EXP uv run python scripts/mib/eval_mib.py --model $model --task $task --steps 2000 --k-schedule uniform --masking hard_topk --mode sufficient --lr 0.01 --split validation --train-split train --include-input $bs --output results/htk_lr_0.01_2k"
  sleep 1
done
