...
# Reproduces jobs from 2026-06-30. Run from repo root on cluster A (SLURM).
#
...
for SCHED in uniform log; do
  ...
  for p in "${PAIRS[@]}"; do
    ...
    # scripts/mib/launch/mib_node_seed.sbatch args: <model> <task> <masking> <kschedule> <outdir> <opt> <seed> <bs>
    sbatch $RES -J idgumb_${tag}_${model}_${task} scripts/mib/launch/mib_node_seed.sbatch \
