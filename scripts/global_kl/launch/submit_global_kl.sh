...
#
#   bash scripts/global_kl/launch/submit_global_kl.sh          # submit all arms
#   DRY=1 bash scripts/global_kl/launch/submit_global_kl.sh    # print instead
#
...
submit () {   # submit <jobname> <args...>
  local name=$1; shift
  if [[ -n ${DRY:-} ]]; then
    echo "sbatch -J $name scripts/global_kl/launch/global_kl.sbatch ${COMMON[*]} $*"
  else
    sbatch -J "$name" scripts/global_kl/launch/global_kl.sbatch "${COMMON[@]}" --reuse-scores "$@"
  fi
}
