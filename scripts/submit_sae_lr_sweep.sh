...
# scripts/compare_sae_ranks.py. An lr that improves acc_auc while still selecting 100% dead
# latents has found a different bug, not fixed this one.
#
#   bash scripts/submit_sae_lr_sweep.sh        # 13 jobs
#   DRY=1 bash scripts/submit_sae_lr_sweep.sh  # print only
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p logs
...
sub() { # $1=jobname $2=outdir ; rest = extra args
  ...
  else mkdir -p "$out"; sbatch -J "$name" sva_sweep.sbatch "${COMMON[@]}" "$@" \
