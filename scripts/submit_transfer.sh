...
#
#   bash scripts/submit_transfer.sh          # submit all 13 jobs (one per TARGET task)
#   DRY=1 bash scripts/submit_transfer.sh    # print instead
#
# METHOD=<m> switches every source to results/transfer_src/<m>/{task}.pt (prepared by
# scripts/prep_transfer_sources.py -- e.g. adam / mc_ig / attnlrp) and suffixes the output
...
#
# MIB targets (ioi/arith-sub/mcqa/arc-e/arc-c) go through scripts/eval_transfer_mib.py
...
if [[ -n "$METHOD" ]]; then
  [[ -d "results/transfer_src/$METHOD" ]] || { echo "no results/transfer_src/$METHOD (run scripts/prep_transfer_sources.py)" >&2; exit 1; }
...
for t in "${MIB_TASKS[@]}"; do
  [[ "$HAVE" == *" $t "* ]] || continue
  sub "xferm${SUF}_${t}" transfer_mib.sbatch \
...
for t in "${SVA_TASKS[@]}" "${ARITH_TASKS[@]}"; do
  ...
  sub "xfers${SUF}_${t}" --time=24:00:00 sva_sweep.sbatch \
