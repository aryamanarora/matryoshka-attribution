...
# decides where the circuit switches on -- so the budget is named in the row label rather
# than left as a hidden default. (label, results dir); the unsuffixed dir is the runner's
# own default (0.9 node / 0.99 edge).
#
# We report only s=0.9, its best budget by CPR (validation avg over 11 cells: 0.9973 at
# s=0.9 vs 0.9573 / 0.9122 at 0.95 / 0.99 -- acc-AUC ranks them the other way, 0.4027 vs
# 0.4582 / 0.4592). The other two dirs below are still on disk and still generated by
# `run_edge_pruning.sbatch ... <sparsity>`; append them here to put those rows back.
# Budgets genuinely disagree with each other (cross-budget rank rho 0.39-0.56), so this
# is a reporting choice, not a cosmetic one.
EPRUN_SPARSITIES = [
    ("0.9", "eprun_eval"),
]
EPRUN_SPARSITIES_ALL = EPRUN_SPARSITIES + [
