"""Full-width facetted scatter plot of node ranks: L2A vs natural-k L2A across all tasks."""
...
def main():
    ...
    for task, model, label in TASKS:
        ...
        l2a_sorted = sorted(common, key=lambda n: l2a[n], reverse=True)
        nk_sorted = sorted(common, key=lambda n: nk[n], reverse=True)
        l2a_rank = {n: i + 1 for i, n in enumerate(l2a_sorted)}
        nk_rank = {n: i + 1 for i, n in enumerate(nk_sorted)}
        ...
        for node in common:
            ...
            all_rows.append({
                "l2a_rank": l2a_rank[node],
                "nk_rank": nk_rank[node],
                "category": cat,
                "type": cat,
                "facet": facet_label,
            })
    ...
    p = (
        ggplot(df, aes(x="l2a_rank", y="nk_rank", color="category"))
        ...
        + labs(x="Rank (L2A)", y="Rank (L2A, natural $k$)", color="")
    ...
    out = Path("paper/figs/rank_scatter_naturalk.pdf")
