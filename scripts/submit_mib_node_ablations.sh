...
for a in "${ABL[@]}"; do
  ...
  for p in "${PAIRS[@]}"; do
    ...
    nlprun -g 1 -q jag -d a6000 $res -n "$name" \
      "PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True uv run python scripts/eval_mib.py --model $model --task $task --steps 500 --k-schedule $ks --masking $mask --mode necessary --split validation --train-split train $bs --output results/$out"
    sleep 1
  done
done
