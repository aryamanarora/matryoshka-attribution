...
#
#   SMOKE=1 bash scripts/submit_sae_noerr.sh   # 1 cheap job, run this FIRST
#   DRY=1   bash scripts/submit_sae_noerr.sh   # print only
#   bash scripts/submit_sae_noerr.sh           # 40 jobs (8 tasks x 5 methods)
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p logs
...
sub() { local name=$1 out=$2; shift 2
  ...
  else mkdir -p "$out"; sbatch -J "$name" sva_sweep.sbatch --model llama3 --nodes "$NODES" \
