...
query-efficiency one. Every method's budget is measured, not assumed (`CountingModel.rows`).
    ...
    uv run python scripts/impossibility/impossibility_bench.py --datasets wine --n-models 2   # pilot
    uv run python scripts/impossibility/impossibility_bench.py                                # full
"""
import argparse
import json
import sys
