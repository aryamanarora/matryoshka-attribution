...
#
# WHY THIS ARM EXISTS. scripts/submit_adam_eps_followup.sh established that Adam's eps is an
...
#
#   bash scripts/submit_sva_eps.sh          # submit missing
#   DRY=1 bash scripts/submit_sva_eps.sh    # print, submit nothing
#   FORCE=1 bash scripts/submit_sva_eps.sh  # resubmit even if the json exists
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p logs
...
sub() {  # $1=jobname $2=outdir $3=predicted-filename ; rest = eval_sva.py args
  ...
  else mkdir -p "$out"; sbatch -J "$name" sva_sweep.sbatch "$@" --output "$out" >/dev/null; fi
  n=$((n+1))
}
