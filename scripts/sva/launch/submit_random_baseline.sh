...
#
#   bash scripts/sva/launch/submit_random_baseline.sh          # submit missing
#   DRY=1 bash scripts/sva/launch/submit_random_baseline.sh    # print only
#   SEEDS="42" bash scripts/sva/launch/submit_random_baseline.sh
set -euo pipefail
cd "$(dirname "$0")/../../.."
mkdir -p logs
...
sub() {  # $1=jobname $2=out-dir $3=predicted-filename-tag ; rest = eval_sva args
  ...
  else sbatch -J "$name" scripts/sva/launch/sva_sweep.sbatch "$@" >/dev/null; fi
  n=$((n+1))
}
