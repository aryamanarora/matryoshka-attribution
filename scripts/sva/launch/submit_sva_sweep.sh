...
#   bash scripts/sva/launch/submit_sva_sweep.sh          # submit missing jobs
#   DRY=1 bash scripts/sva/launch/submit_sva_sweep.sh    # print, submit nothing
#   FORCE=1 bash scripts/sva/launch/submit_sva_sweep.sh  # resubmit even if output exists
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
#   OUT=results/sva_zeroabl ABLATION=zero bash scripts/sva/launch/submit_sva_sweep.sh
...
# patch-vs-zero comparison in scripts/sva/compare_ablation.py, which matches the two settings by
...
submit() {   # $1=name $2=tag ; rest = eval_sva.py args
  ...
  if [[ "${DRY:-0}" == "1" ]]; then echo "sbatch -J $name scripts/sva/launch/sva_sweep.sbatch $*"
  else sbatch -J "$name" scripts/sva/launch/sva_sweep.sbatch "$@"; fi
  n=$((n+1))
}
...
#   MIB_TASKS="arc_easy" IG_STEPS=5 bash scripts/sva/launch/submit_sva_sweep.sh
...
#   ARITH_TASKS="addition months weekdays hours" IG_STEPS=5 bash scripts/sva/launch/submit_sva_sweep.sh
