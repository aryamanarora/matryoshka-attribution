...
#
#   bash scripts/sva/launch/submit_sae_budget.sh        # 32 jobs
#   DRY=1 bash scripts/sva/launch/submit_sae_budget.sh  # print only
set -euo pipefail
cd "$(dirname "$0")/../../.."
mkdir -p logs
...
sub() { # $1=name ; rest = args
  ...
  else sbatch -J "$name" scripts/sva/launch/sva_sweep.sbatch "$@" --output "$OUT" >/dev/null; fi
  n=$((n+1))
}
for t in "${TASKS[@]}"; do
  d=${DS[$t]}
