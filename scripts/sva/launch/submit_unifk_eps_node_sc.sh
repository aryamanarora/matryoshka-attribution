...
# (results/sva_sweep/<task>_<model>_node_iso_topk_adam_eps1e-2[_<loss>]_bs1.json):
...
#   <task>_<model>_node_iso_topk_adam_eps1e-2[_<loss>]_uniformk_bs1.json
#
# cluster B / gpujob, run INSIDE tmux. ioi is qwen2.5 (V.on_model), everything else llama3.
...
COMMON="--nodes node --method mattr --variant topk --mode iso --optimizer adam --adam-eps 1e-2 \
...
for task in $TASKS; do
  ...
  for loss in $LOSSES; do
    ...
    f="$OUT/${task}_${model}_node_iso_topk_adam_eps1e-2${ls}_uniformk_bs1.json"
    if [ -f "$f" ]; then echo "SKIP $(basename $f)"; skip=$((skip+1)); continue; fi
    name="unifeps-node-${task}-${loss}"
    # -d a6000 on BOTH: an unpinned gpu-queue job can land on a GPU too old for this torch build
    # ("no kernel image is available for execution on the device" -- the ioi/ce arm did, 2026-09-15).
    [ "$model" = qwen2.5 ] && res="-q gpu -d a6000 -c 2 -r 32G" || res="-q gpu -d a6000 -c 4 -r 96G"
