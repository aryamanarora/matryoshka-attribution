...
# The 5k runs took 7-10 min (MLP), 8-10 min (MLP+Attn), 15-35 min (SAE) on cluster A, so expect
# ~1.5 h / ~1.5 h / 3-6 h per cell here. cluster B / gpujob, run INSIDE tmux.
...
# minutes, 2026-09-17); the 5k headline ran on cluster A's 80 GB H100s. large-memory h100 for those.
RES="-q gpu -d a6000 -c 4 -r 96G"; RES_SAE="-q gpu-big -d h100 -r 128G"
n=0; skip=0
for t in $TASKS; do
