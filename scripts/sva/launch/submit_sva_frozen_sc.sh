...
for nodes in $NODES; do
  ...
  for ks in $KS; do
    ...
    for task in $tasks; do
      ...
      f="$out/${task}_${model}_${nd}_sufficient_topk_none${ktag}${ferr}_bs1${ssuf}.json"
...
--model $model --task $task --dataset $ds --nodes $nodes --method mattr --variant topk --mode sufficient \
