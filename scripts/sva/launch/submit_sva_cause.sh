...
#
# WHY: all 720 runs in results/sva_sweep{,_input} are --mode iso, i.e. the mask is
...
# for. This script trains the SAME method with --mode cause so the comparison is
# like-for-like.
#
...
# conductance rankings are byte-identical under --mode cause and re-running them would
# only produce differently-named copies. Evaluation is direction-independent too: summarize()
# sets hooker.corrupt_topk explicitly per call, and F_clean/F_patch always use corrupt_topk=False.
#
...
# in the filename tag `cause_...`, but a separate dir also keeps these out of every glob
# that feeds the sufficiency figures. plot_accauc_vs_faithauc.parse_method now returns None for
# `cause_*` rather than silently labelling it "IG" -- do not undo that.)
#
#   bash scripts/sva/launch/submit_sva_cause.sh          # submit missing jobs
#   DRY=1 bash scripts/sva/launch/submit_sva_cause.sh    # print, submit nothing
#   FORCE=1 bash scripts/sva/launch/submit_sva_cause.sh  # resubmit even if output exists
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
#   VARIANTS="topk:adam hard_topk:adam" bash scripts/sva/launch/submit_sva_cause.sh
read -ra VARIANTS <<< "${VARIANTS:-topk:adam}"
STEPS=${STEPS:-2000}
COMMON=(--mode cause --k-schedule log --train-batch-size 1 --steps "$STEPS" \
