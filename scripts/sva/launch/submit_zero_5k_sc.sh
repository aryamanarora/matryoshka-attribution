...
# exists is skipped). SAE on large-memory h100 (the encode OOMs a 48 GB a6000). cluster B / gpujob, in tmux.
...
for nodes in $NODES; do
  ...
  for task in $tasks; do
    ...
    for m in $METHODS; do
      ...
      if [ $sae = 1 ]; then res="-q gpu-big -d h100 -r 128G"
      elif [ "$model" = qwen2.5 ]; then res="-q gpu -d a6000 -c 2 -r 32G"
      else res="-q gpu -d a6000 -c 4 -r 96G"; fi
      name="z5k-$m-$nd-$task"
