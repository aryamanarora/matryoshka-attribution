...
#   bash scripts/sva/launch/submit_input_replication.sh          # submit missing
#   DRY=1 bash scripts/sva/launch/submit_input_replication.sh    # print only
#
...
#
#   ONLY=softsgd bash scripts/sva/launch/submit_input_replication.sh                      # 30 jobs
#   ONLY="ig ixg softsgd" OUT=results/sva_zeroabl_input ABLATION=zero \
#     bash scripts/sva/launch/submit_input_replication.sh                                 # 90 jobs
#
...
sub() {  # $1=jobname $2=predicted-filename-tag ; rest = eval_sva args
  ...
  if [[ "${DRY:-0}" == 1 ]]; then echo "sbatch -J $name ... $tag"; else sbatch -J "$name" scripts/sva/launch/sva_sweep.sbatch "$@" >/dev/null; fi
  n=$((n+1))
}
