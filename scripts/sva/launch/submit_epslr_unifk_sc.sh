...
#
# cluster B / gpujob, run INSIDE tmux. gemma2 is fine in the L2A env here (eval_sva loads through
...
row_spec() {  # -> "model nodes errarg outbase prefix res"
  case "$1" in
    node)        echo "llama3 node - results/epslr_node_unifk ukn -q_gpu_-d_a6000_-c_4_-r_96G" ;;
    mlpn_gemma2) echo "gemma2 mlp - results/epslr_mlpn_gemma2_unifk ukmg -q_gpu_-d_a6000_-c_3_-r_64G" ;;
    mlpn)        echo "llama3 mlp - results/epslr_mlpn_unifk ukm -q_gpu_-d_a6000_-c_4_-r_96G" ;;
    # large-memory h100, not a6000: Llama-8B + 32 Llama-Scope SAEs (5.2M mask logits) OOMs a 48 GB
    # card at the first training step (2026-09-16); the log-k twin ran on cluster A's 80 GB H100s.
    sae)         echo "llama3 mlp_sae_span frozen results/saefrozen_epslr_unifk uks -q_bigmem_-d_h100_-r_128G" ;;
...
--train-batch-size 1 --steps $STEPS --eval-examples 100 --output $out"
      n=$((n+1))
      if [ "$DRY" = 1 ]; then echo "DRY gpujob -g 1 $res -n $name"; else
        gpujob -g 1 $res -n "$name" -o "$ABS/logs/${name}.out" "$cmd" 2>&1 | grep -E 'Submitted batch job' | sed "s/^/$name: /"; sleep 1; fi
    fi
  fi
