...
shows dead@10k near 1.0, the diagnosis in scripts/sva/launch/submit_sae_lr_sweep.sh is wrong and the score
...
Run:  uv run python scripts/sva/collect_sae_lr_sweep.py
"""
import glob
import json
import os
...
def main():
    ...
    if not dirs:
        raise SystemExit(f"no sweep dirs under {ROOT} -- run scripts/sva/launch/submit_sae_lr_sweep.sh")
