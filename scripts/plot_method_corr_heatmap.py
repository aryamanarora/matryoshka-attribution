...
METHODS = [
    ...
    ("MAttr (log)",    "mib_node_hard_topk_log",                        "flat"),
    ("+Gumbel",        "mib_node_hard_topk_gumbel",                     "flat"),
    ("+soft",          "final_node",                                    "flat"),
    ("+soft (log)",    "mib_node_topk_log",                             "flat"),
    ("$-c_k$",         "mib_node_detached_tau",                         "flat"),
    ("$-c_k$ (log)",   "mib_node_detached_tau_log",                     "flat"),
    ("+hard",          "mib_node_bernoulli_reinforce",                  "flat"),
    ("+hard (log)",    "mib_node_bernoulli_reinforce_log",              "flat"),
...
p = (ggplot(df, aes("a", "b", fill="rho")) + geom_tile(color="white")
     ...
     + theme(figure_size=(7.2, 6.2), panel_grid=element_blank(),
             axis_text_x=element_text(rotation=45, ha="right", size=6),
             axis_text_y=element_text(size=6)))
p.save(OUT / "method_corr_heatmap.png", dpi=150); p.save(OUT / "method_corr_heatmap.pdf")
...
p2 = (ggplot(fd, aes("a", "b", fill="rho")) + geom_tile()
      ...
      + theme(figure_size=(13, 9), panel_grid=element_blank(),
              axis_text_x=element_text(rotation=90, size=3.5),
              axis_text_y=element_text(size=3.5), strip_text=element_text(size=7)))
