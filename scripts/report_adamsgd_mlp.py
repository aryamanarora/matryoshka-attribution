...
copied number goes stale silently (see CLAUDE.md, "Verification anchor").
    ...
    uv run python scripts/report_adamsgd_mlp.py      # then this
...
def build():
    ...
    font-size:.8rem;overflow-x:auto">bash scripts/submit_adam_vs_sgd_mlp.sh        # arms A–D (eps, losses, fixed-k, per-example)
bash scripts/submit_adam_eps_followup.sh      # eps top end, nounpp, node control, grad stats
...
uv run python scripts/report_adamsgd_mlp.py   # this page</pre>""")
