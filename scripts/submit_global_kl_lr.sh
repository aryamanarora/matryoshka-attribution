...
#
#   bash scripts/submit_global_kl_lr.sh          # submit
#   DRY=1 bash scripts/submit_global_kl_lr.sh    # print
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p logs
...
for lr in $SGD_LRS; do
  cmd=(sbatch -J "gklr_sgd$lr" global_kl.sbatch "${COMMON[@]}" --optimizer sgd --lr "$lr")
  [[ -n ${DRY:-} ]] && echo "${cmd[*]}" || "${cmd[@]}"
done
for lr in $ADAM_LRS; do
  cmd=(sbatch -J "gklr_adam$lr" global_kl.sbatch "${COMMON[@]}" --optimizer adam --lr "$lr")
  [[ -n ${DRY:-} ]] && echo "${cmd[*]}" || "${cmd[@]}"
done
