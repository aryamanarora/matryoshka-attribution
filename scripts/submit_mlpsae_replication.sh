...
#
#   bash scripts/submit_mlpsae_replication.sh        # 12 jobs
#   DRY=1 bash scripts/submit_mlpsae_replication.sh  # print only
set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p logs
...
sub() { local name=$1 out=$2; shift 2
  ...
  else mkdir -p "$out"; sbatch -J "$name" sva_sweep.sbatch "${COMMON[@]}" "$@" --output "$out" >/dev/null; fi
  n=$((n+1)); }
...
if [[ "${DRY:-0}" == 1 ]]; then echo "  mlpsae_probe -> $ROOT/probe (MATTR_LOG_EVERY=1)"; n=$((n+1))
  ...
  sbatch -J mlpsae_probe --export=ALL,MATTR_LOG_EVERY=1 sva_sweep.sbatch "${COMMON[@]}" \
