#!/bin/bash
# The 3 missing `MAttr (SGD)` runs for IOI/qwen2.5 in results/sva_sweep (one per loss).
#
...
# makes this hole visible instead of silently filled by the wrong model. These 3 jobs close it.
#
...
for loss in ce acc logit_diff; do
  ...
  tag="sufficient_topk_sgd${ls}_bs1"
  name="sgdq_ioi_${loss}"
  ...
  args=(--model qwen2.5 --task ioi --dataset mib --nodes node --eval-examples 100
        ...
        --optimizer sgd --k-schedule log --mode sufficient --train-batch-size 1
        --steps 2000 --lr 1.0)
