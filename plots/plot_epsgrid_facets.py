...
COLS = [
    ...
    # vmax 7.0: the linear CPR reaches 5.97 (log-k) / 6.82 (uniform-k) on the Gemma MLP row
    # (gap-padding at small eps); the TwoSlopeNorm keeps white at 1.0, so only the red arm
    # stretches. One scale for both twins so they can be read against each other.
    ("cpr", "CPR", "RdBu_r", 0.0, 7.0, None),
